...
| `--branch` | `main` | Tên nhánh Git muốn thực hiện push. |
...
3. Đăng ký một Windows Scheduled Task có tên là `GitAutoCommitPush` tự động lặp lại mỗi 60 giây.
...
4. Ở tab **Triggers**: Thêm trigger mới, chọn chạy **Once** và tích chọn **Repeat task every:** `1 hour` hoặc `1 minute` tùy nhu cầu, mục **for a duration of:** chọn `Indefinitely` (vô hạn).
5. Ở tab **Actions**: Thêm Action mới loại **Start a program**:
   * **Program/script**: `poetry`
   * **Add arguments**: `run git-auto-commit --repo C:\path\to\your\repo --interval 60 --branch main`
   * **Start in**: `C:\path\to\your\repo` (đường dẫn tới thư mục chứa mã nguồn công cụ auto-commit này).
